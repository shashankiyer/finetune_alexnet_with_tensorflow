...
def reli(threshold, k, query_emb_bin, query_emb_float, query_lab, database_emb_bin, database_emb_float, database_lab):
    ...
    #res = np.apply_along_axis(reli_image_wise, 1, hamming, threshold, k, query_emb_float, database_emb_float, query_lab, database_lab)
    ...
    while not it.finished:
        it[0] = reli_image_wise(k , query_emb_float[it.index], np.where(hamming[it.index] <= threshold)[0], database_emb_float, query_lab[it.index], database_lab)
        it.iternext()
